...
`➡️ /modele/goodMoulde/pages/detail/detail.js`
...
> 在这个例子中，`cartList`可能是一个数组，而通过使用方括号包裹的表达式`cartList[${index}].isChecked`，可以动态地访问`cartList`数组中特定索引（由变量`index`决定）的元素的`isChecked`属性。这样就可以根据不同的情况动态地更新特定索引位置的对象的这个属性值。
>
> 如果不使用方括号这种动态表达式的方式，就无法灵活地针对数组中的特定元素的特定属性进行更新操作。在 JavaScript 中，利用计算属性名的语法通常是写在方括号里面。
