...
> ```
>
> 可以将两个数组（当前的`orderList`和从接口响应中获取的`res.data.records`）合并成一个新的数组。这种方式简洁直观，避免了使用传统的循环或其他复杂的数组操作方法。
>
> 例如，如果 `this.data.orderList` 是 `[1, 2, 3]`，`res.data.records` 是 `[4, 5, 6]`，那么合并后的结果就是 `[1, 2, 3, 4, 5, 6]`。
>
> 
>
> 扩展运算符不一定必须放在方括号里，但放在方括号里通常是用于数组的操作，比如合并数组等。
> 如果放在花括号里，一般是用于对象的解构赋值或对象字面量的属性值扩展等情况。
