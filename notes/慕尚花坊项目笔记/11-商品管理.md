...
> 在`  goodsList: [...this.data.goodsList, ...data.records]`中，`goodsList`的赋值使用了扩展运算符`...`，有以下原因：
>
> 1. ```js
>    ...this.data.goodsList
>    ```
>
>    这里的扩展运算符用于将原有的`goodsList`数组展开成一个个独立的元素。这样做的目的是创建一个新的数组，包含原`goodsList`中的所有元素，而不是直接引用原数组。如果直接使用`this.data.goodsList`进行赋值，可能会导致对原数组的意外修改，而通过展开的方式创建新数组，可以确保原数组不被直接修改，提高代码的安全性和可维护性。
>
> 2. ```js
>    ...data.records
>    ```
>
>    同样，这个扩展运算符将`data.records`数组展开。这样做是为了将`data.records`中的元素逐个添加到新的数组中，与原有的`goodsList`元素合并在一起。如果不使用扩展运算符，可能需要使用循环等方式逐个添加元素，代码会更加复杂。
...
> ```javascript
> // 将商品 id 挂载到页面实例上this.goodsId = options.goodsId ? options.goodsId : ''
> ```
>
> “挂载到页面实例上” 意味着将一个属性直接添加到页面实例对象上，而不是作为初始定义的`data`中的属性。这样做可以方便地<u>在页面的不同方法和生命周期函数中访问这个属性，而不必局限于通过`this.data`来访问。</u>
>
> 通常情况下，页面的`data`属性用于存储与页面显示相关的数据，这些数据会在页面渲染时被使用，并且可以通过数据绑定机制自动更新页面。而挂载到页面实例上的属性可以用于存储一些临时的、特定场景下的信息，或者是从页面路由参数中获取的信息，<u>这些信息可能不需要参与页面的渲染过程</u>，但在页面的逻辑处理中需要用到。
