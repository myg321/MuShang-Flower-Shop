...
但是部分接口如 `downloadFile`, `request`, `uploadFile` 等本身就有返回值，因此不支持 promise 调用方式，无法使用 `.then()` 和 `.catch()` 来处理结果, 它们的 promisify 需要开发者自行封装。
...
> ```
>
> 如果要将其封装为 Promise 方式，可以这样做：
>
> ```js
> function promisifiedDownloadFile(url) {
...
> 
> // 使用封装后的函数
> promisifiedDownloadFile('https://example.com/file.jpg').then((filePath) => {
>   console.log('下载成功的文件路径', filePath);
> }).catch((err) => {
>   console.error('下载失败', err);
> });
> ```
...
`Axios` 是我们日常开发中常用的一个基于 promise 的网络请求库, 意味着它的网络请求方法（如 `axios.get()`、`axios.post()` 等）都会返回一个 Promise 对象。
...
>   ```
>
>   - 导入时必须与导出的名称完全匹配，并且需要使用花括号包裹多个命名导出。
>   - 如果导出的名称发生变化，导入的地方也需要相应修改。
>
> - `export default`：
>
>   - 导入时可以使用任意名称，更加灵活。
>   - 即使默认导出的名称在源模块中发生变化，导入的地方通常不需要修改（只要导入的名称不冲突）。
...
> 在 JavaScript 中，`{...this.defaults,...options }` 这种语法是对象展开语法，用于合并对象。
>
> 1. **展开语法原理**
>    - 当使用对象展开语法时，它会将对象中的可枚举属性逐个展开。
>    - 对于 `{...this.defaults,...options }`，它首先会展开 `this.defaults` 对象的所有属性，然后再展开 `options` 对象的所有属性。如果 `options` 对象中的属性与 `this.defaults` 对象中的属性有相同的键名，那么 `options` 对象中的属性值会覆盖 `this.defaults` 对象中的属性值。
> 2. **示例说明**
>    - 假设 `this.defaults = {a: 1, b: 2}`，`options = {b: 3, c: 4}`。
>    - 那么 `{...this.defaults,...options }` 的结果是 `{a: 1, b: 3, c: 4}`。
>    - 首先 `this.defaults` 的属性 `a` 和 `b` 被展开，然后 `options` 的属性 `b`（覆盖了前面的 `b` 值）和 `c` 被展开。
...
**响应**拦截器本质上是在响应之后调用的函数，用来对响应数据做点什么
...
>
>    当有新的请求发起时，先检查是否存在之前设置的定时器（`this.timerId`）。如果存在，就使用`clearTimeout`取消这个定时器。这是为了确保在新请求到来时，不会因为旧的定时器而导致不期望的行为。例如，如果旧定时器即将触发隐藏加载提示，但新请求来了，此时应该取消旧定时器，根据新请求的情况重新处理加载提示的显示和隐藏。
>
>    
>
> 2. `this.queue.length === 0 && wx.showLoading()`：
>
>    当请求队列长度为 0 时，表示当前没有正在进行的请求。在这种情况下，显示加载提示（`wx.showLoading`）。这是为了在发起第一个请求或者所有请求都完成后再次发起请求时，及时向用户显示正在加载的状态。
>
>    
>
> 3. `this.queue.push('request')`：
>
>    在发送请求之前，向队列中添加一个`'request'`标识。这个标识代表有一个新的请求正在进行中。通过维护这个队列，可以跟踪当前正在进行的请求数量。
>
>    
>
> 4. `this.queue.pop()`
>
>    complete回调中，每次执行完一个request就pop出一个队列中的'request''标识。
>
>    
>
> 5. `this.queue.length === 0 && this.queue.push('request')`
>
>    还是complete回调中, 每次pop后检查队列中是否还有'request'标识, 如果没了, 为了应对可能一个请求依赖于另一个请求的情况, 先主动再在队列加入一个'request'标识来表示即将进行的依赖着前一个请求的请求, 如果真的有这个新请求, 则它会执行`1.`中清除定时器代码, 不执行`2.`代码(没满足条件), 执行`3.`的push一个请求的代码, 这个请求完成后执行`4.`的pop代码 (此时队列中还有一个'request'), 之后执行下面的`setTimeout`方法.
>
>    
>
> 6. `this.timerId = setTimeout(() => {...}, 100)`：
>
>    定时器的回调函数中先从队列中删除一个'request'（如果后面再没新的请求了,那么这这步操作后, 队列就没任何'request'了），然后检查队列长度是否为 0, 如果是 0 则隐藏加载提示。
>
>    不过这两行代码需要等待 100ms 后执行, 如果在这 100ms 内又有新的请求出现, 则新请求依然会像`5.`中所说, 删除定时器 -> push一个请求 -> 完成后pop一个请求 -> 再次进入`setTimeout`方法
...
> **啥时候用冒号`:` 啥时候用等号`=`？**
>
> - 冒号（“:”）在对象字面量中用于表示属性名和属性值的对应关系，比如在定义对象时`{ propertyName: propertyValue }`。
> - 等号（“=”）赋值的时候使用。
