...
4. 此时就已经创建了一个功能组件
   ...
   ![](http://8.131.91.46:6677/mina/base/创建和注册组件.png)
...
在使用基础组件时，可以给组件传递<u>子节点</u>传递内容，从而将内容展示到页面中，自定义组件也可以接收子节点内容
...
3. 生命周期函数和 observers 不会相互覆盖，会是在对应触发时机被逐个调用，也就是<u>都会被执行</u>
