...
Page({
  ...
  async delHandler () {
    ...
    // showModal 显示模态对话框
    // 这里是把返回对象Promise（包含cancel、confirm）进行了解构，只提取其confirm用于后续判断
...
```html
<button type="primary" bindtap="handler" size="mini" plain bindtap="setStorage">
  存储数据
</button>

<button type="primary" bindtap="handler" size="mini" plain bindtap="getStorage">
  获取数据
</button>

<button type="warn" bindtap="handler" size="mini" plain bindtap="delStorage">
  删除数据
</button>

<button type="warn" bindtap="handler" size="mini" plain bindtap="clearStorage">
  移除数据
</button>
```
...
2. 编程式导航：使用小程序提供的 `API`
   ...
   - `wx.reLaunch()`：关闭所有页面，之后跳转到应用中某一页面
...
​                    <img src="http://8.131.91.46:6677/mina/base/上拉加载更多案例.gif" style="zoom:80%;" />
...
bindscrolltolower：滚动到底部/右边时触发，属性值为自定义的 下拉刷新函数名
