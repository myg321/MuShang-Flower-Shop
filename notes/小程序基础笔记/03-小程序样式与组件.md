...
在 CSS 中，伪元素是一种特殊的选择器，可以用来对特定元素的特定部分进行样式设置，而这些部分在文档中并不实际存在，是通过 CSS 生成的虚拟元素。
...
1. 创建额外的装饰元素
   - 比如可以使用 `::before` 和 `::after` 伪元素在一个元素的内容前后添加额外的图形、图标或装饰线条等。例如，可以在一个标题元素前添加一个装饰图标，增强页面的视觉效果。
   - 代码示例：
...
在这个例子中，每个 `<h1>` 元素的前面会出现一个金色的五角星图标。



2. 制作特殊的边框效果

   - 伪元素可以用来创建独特的边框效果，而不仅仅局限于使用 `border` 属性。例如，可以使用 `::before` 和 `::after` 伪元素在元素周围创建双层边框效果。
...
这段代码会在一个类名为 `.box` 的元素周围创建一个蓝色的双层边框效果。
...
1. 清除浮动
   - 在网页布局中，当使用浮动元素时，可能会导致父元素高度塌陷。可以使用 `::after` 伪元素来清除浮动，确保父元素能够正确地包含浮动的子元素。
   - 代码示例：
...
将这个类 `.clearfix` 应用到包含浮动元素的父元素上，可以解决高度塌陷问题。



2. 首行缩进

   - 可以使用 `::first-line` 伪元素来对文本的首行进行特殊的样式设置，比如首行缩进。
